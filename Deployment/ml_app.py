...
def predict_price(Area_SqFt,Floor_No,Bedroom,Bathroom):
    x = np.zeros(8)
...
def run_ml_app():
    ...
    if st.button("Calculate Price"):
        result = predict_price(Area_SqFt,Floor_No,Bedroom,Bathroom)
    st.success('Total Price in USD : {}'.format(result))
